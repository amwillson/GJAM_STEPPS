...
## Beta regression with UNstandardized relative abundance
## estimated in the model but pooled alpha coefficients
## describing the relationship between previous and current
## relative abundance
...
for(s in 1:ns){
# Process equation
logit(mu[t,s]) <- x[t-1,s] + alpha * x[t-1,s] + beta[1] * sand[t,s] + beta[2] * aat[t,s] + beta[3] * tpr[t,s] + beta[4] * prsd[t,s]
...
jm <- rjags::jags.model(file = textConnection(dlm_beta_all_pooled_nostand),
                        data = data,
                        n.chains = 3,
                        n.adapt = 1000)
...
save(jm, hm_out_pooled, hm_out_alpha,
     file = 'out/DLM/hemlock_mountain_pooled_unstand.RData')
